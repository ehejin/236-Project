...
from ..encoder import SchNetVAE, GINEncoder, get_edge_encoder
...
class DualEncoderEpsNetwork(nn.Module):
    ...
    def forward(self, atom_type, pos, bond_index, bond_type, batch, time_step,
        ...
        # Global
        node_attr_global, reconstructed_global, mu_global, logvar_global = self.encoder_global(
            z=atom_type,
        ...
        if return_edges:
            return edge_inv_global, edge_inv_local, edge_index, edge_type, edge_length, local_edge_mask, (reconstructed_global, mu_global, logvar_global, node_attr_global)
        else:
            return edge_inv_global, edge_inv_local, (reconstructed_global, mu_global, logvar_global, node_attr_global)

    def get_loss(self, atom_type, pos, bond_index, bond_type, batch, num_nodes_per_graph, num_graphs,
                 anneal_power=2.0, return_unreduced_loss=False, return_unreduced_edge_loss=False, extend_order=True,
                 extend_radius=True, is_sidechain=None):
        if self.model_type == 'diffusion':
            return self.get_loss_diffusion(atom_type, pos, bond_index, bond_type, batch, num_nodes_per_graph,
                                           num_graphs,
                                           anneal_power, return_unreduced_loss, return_unreduced_edge_loss,
    ...
    def get_loss_diffusion(self, atom_type, pos, bond_index, bond_type, batch, num_nodes_per_graph, num_graphs,
        ...
        edge_inv_global, edge_inv_local, edge_index, edge_type, edge_length, local_edge_mask, vae_outputs = self(
            atom_type=atom_type,
            pos=pos_perturbed,
        ...
        reconstructed_global, mu_global, logvar_global, node_attr_global = vae_outputs
        recon_loss_global = torch.nn.functional.mse_loss(reconstructed_global, node_attr_global)
        kl_div_global = -0.5 * torch.sum(1 + logvar_global - mu_global.pow(2) -logvar_global.exp())
        ...
        node_eq_local = eq_transform(edge_inv_local, pos_perturbed, edge_index[:, local_edge_mask],
                                     edge_length[local_edge_mask])
        ...
        loss = loss_global + loss_local + recon_loss_global + kl_div_global
    ...
    def langevin_dynamics_sample(self, atom_type, pos_init, bond_index, bond_type, batch, num_graphs, extend_order,
                                 extend_radius=True,
        ...
        if self.model_type == 'diffusion':
            return self.langevin_dynamics_sample_diffusion(atom_type, pos_init, bond_index, bond_type, batch,
                                                           num_graphs, extend_order, extend_radius,
            ...
            return self.langevin_dynamics_sample_dsm(atom_type, pos_init, bond_index, bond_type, batch, num_graphs,
                                                     extend_order, extend_radius,
                                                     n_steps, step_lr, clip, clip_local, clip_pos, min_sigma,
                                                     is_sidechain,
                                                     global_start_sigma, w_global, w_reg)

    def langevin_dynamics_sample_diffusion(self, atom_type, pos_init, bond_index, bond_type, batch, num_graphs,
    ...
    def langevin_dynamics_sample_dsm(self, atom_type, pos_init, bond_index, bond_type, batch, num_graphs, extend_order,
                                     extend_radius=True,
                                     n_steps=100, step_lr=0.0000010, clip=1000, clip_local=None, clip_pos=None,
                                     min_sigma=0, is_sidechain=None,
